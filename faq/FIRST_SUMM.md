...
# React 如何用某种手段保存登录状态（Cookie 不安全）？
...
# React Native 出现的原因（为了解决什么问题）？

1. 兼容问题: 一份代码多个平台通用，有利于节省成本，缩短开发周期，减少维护工作。开发效率明显提升。
2. 性能问题: H5 应用或者混合应用的运行效率始终比不上原生应用，RN 使用的 UI 组件和原生保持一致，是一个真正的移动应用。
3. 调试方便: 原生调试需要重新编译，效率往往比较低，RN 采用 hotReload 的方式大大提高了开发效率（但是运行效率较低）。
4. 热更新: 频繁的 APP 升级会让用户感到烦躁，还需要进行多次应用商店审核，热更新化解了这一矛盾，只需从云端下载文件更新即可。

# 使用过的 React Native 第三方控件库。
...
# React 中 class 出现的原因（和 function 有什么区别）？
...
# 在使用 React Native 中遇到了什么困难，为什么部分控件只有一个系统可以用？
...
# setState 是异步的吗？为什么情况下需要用同步的 setState？
...
3. 多重继承: Javascript 本身不提供多重继承的方法，但可以通过混入（Mixin）的方式实现（有风险）。

# React 在什么情况下会使用 Hook、高阶组件等？
