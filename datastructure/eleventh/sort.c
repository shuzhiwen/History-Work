...
void insertion_sort_basical(int *a, int n, int d,
...
void shell_sort(int *a, int n, int (*comp)(const void *, const void *)) {
	int prime[15] = {1, 3, 7, 17, 31, 67, 113, 227, 397, 499, 
	                 971, 1667, 3049, 6133, 10799};
...
void quick_sort(int *a, int n, int (*comp)(const void *, const void *)) {
	...
	while (left <= right) {
		while (comp(left, a) < 0 && left - a < n - 1) { left++; }
		while (comp(right, a) > 0 && right - a > 0) { right--; }
...
void merge_sort_basical(int *a, int n1, int *b, int n2,
                        int (*comp)(const void *, const void *)) {
...
int distribution(int *a, int n, int x, struct List *l,
                  int (*comp)(const void *, const void *)) {
  int flag = 0;
