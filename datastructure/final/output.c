...
//更新图的信息
void relax(struct trietree **path, struct Vector *vec, struct trietree *cur, 
           int time_limit, char *req, int *total) {
  int interim, target;
  double tmp = INF;
  struct ListNode *p;
...
//更新时间限制
void update_time_limit(struct Vector *vec, int *time_limit, 
                       struct trietree **path, int next_num) {
...
void show_ans(struct trietree *root, struct Vector *vec,
  ...
  for (int i = 0; i < stats; i++) { path[i] = NULL; }
  for (int i = 0; i < stats; i++) { total[i] = INF; }
  for (int i = 0; i < stats; i++) { visited[i] = false; }
...
//输出窗口的初始化
void ans_print(struct trietree *root, struct Vector *vec, 
               struct trietree **path, int *total, int n,
               char *ear_time, char *begin, char *end, char *req) {
  system("clear");
...
//计算总时间，总花费，总中转次数
void time_fare_tran(struct trietree **path, struct Vector *vec, 
                    int *total, char *req, int begin, int end, int n) {
...
void road_print(struct route *road, int n, int fare, int time_ans, int tran) {
  for (int i = n - 1; i >= 0; i--) {
    printf("\tStep %d: Train: %s   %s => %s\n", n - i, 
            road[i].tra_num, road[i].from, road[i].to);
    printf("\t        Time:  %s --- %s\n", 
