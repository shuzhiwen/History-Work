...
enum Status vector_init(struct Vector *vec, int cap) {
  ...
  vec->buffer = (Vec_DataType *)malloc(cap * sizeof(Vec_DataType));
  if (vec->buffer == NULL) {
...
enum Status vector_reserve(struct Vector *vec, int new_cap) {
  ...
  Vec_DataType *p = (Vec_DataType *)realloc(vec->buffer, new_cap * sizeof(Vec_DataType));
  if (p == NULL) {
    return STATUS_ERR_ALLOC;
  }
...
int vector_find_next(struct Vector *vec, Vec_DataType value, int start_pos) {
  ...
  for (i = start_pos; i < vec->size; i++) {
    if (strcmp(vec->buffer[i].eng_name, value.eng_name) == 0) {
      return i;
    }
  }
...
void vector_reverse(struct Vector *vec) {
  ...
  for (i = 0; i < vec->size / 2; i++) {
    SWAP(vec->buffer[i], vec->buffer[vec->size - i - 1], Vec_DataType);
  }
}
...
void vector_swap(struct Vector *a, struct Vector *b) {
  ...
  SWAP(a->buffer, b->buffer, Vec_DataType *);
}
