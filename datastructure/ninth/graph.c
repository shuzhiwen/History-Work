...
void graph_dijikstra(struct Graph *g, int v) {
  ...
  for (int i = 0; i < g->v; i++) { g->dists[i] = INF; }
  ...
  g->dists[now] = 0;
    ...
    visited[now] = true;
      ...
      if (dis < g->dists[i]) { g->dists[i] = dis; }
    }
...
void graph_update_flow(struct Graph *g, struct Graph *flow, int s, int t) {
  ...
  while (begin != s) {
    ...
    if (max_flow > data) { max_flow = data; }
...
DataType graph_ford_fulkerson(struct Graph *g, struct Graph *flow) {
  ...
  copy_matrix(&flow->m, &g->m);
	...
	for (int i = 0; i < g->v; i++) {
		if (graph_in_degree(g, i) == 0) { s = i; }
		if (graph_out_degree(g, i) == 0) { t = i; }
	}

  while (true) {
    graph_dfs(flow, s);

    if (flow->marked[t] == false) { break; }
