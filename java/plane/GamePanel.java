	...
	public void handleCollision(){
		...
		for(i=0; i<numOfBullets; i++)
			for(j=0; j<numOfEnemyBullets; j++) {
				if(bullets[i].isInScreen() && enemybullets[j].isInScreen()) {
					if(checkCollision(bullets[i].getCenterX(), bullets[i].getCenterY(),
					   enemybullets[j].getCenterX(), enemybullets[j].getCenterY())){
						bullets[i].setOutScreen();
		...
		for(i=0; i<numOfEnemyBullets; i++) {
			if(enemybullets[i].isInScreen() && player.isAlive()) {
				if(checkCollision(player.getCenterX(), player.getCenterY(),
				enemybullets[i].getCenterX(), enemybullets[i].getCenterY())){
					enemybullets[i].setOutScreen();
					player.setDead();
					gui.updateLife(player.getLife());
					score+=10;
  ...
  public void updateBullets() {
	...
	public void shootEnemyBullets(){
		...
		for (int enemyIndex = 0; enemyIndex < numOfEnemy; enemyIndex++) {
			...
			if(probability <= 1 && enemies[enemyIndex].isAlive() && 
			   enemies[enemyIndex].getCenterY() > 0){
				int j ;
				for(j=0; j<numOfEnemyBullets; j++) {
					if(enemybullets[j].isOutScreen())
						break;
				}
        enemybullets[j].shoot(enemies[enemyIndex].getX(),
                              enemies[enemyIndex].getY());
			}
    }
	}
